...
def Train(model, train_DL, val_DL, criterion, optimizer, scheduler,
    ...
    #multiclass_functions1과 다르게 val추가, acc_history도 확인
    loss_history = {"train": [], "val": []} 
    acc_history = {"train": [], "val": []}
    best_loss = 9999 #모델이 학습시 좋은 성능모델 save할떄 val_loss를 가지고 낮을떄를 best_loss와 비교해서 best_loss를 갱신하기떄문에 처음에 높게 잡는다.
    for ep in range(EPOCH):
        epoch_start = time.time() #어느정도 진전인지 초시계 시작
        ...
        with torch.no_grad():
            ...
            #val_loss가 best_loss보다 낮으면 best_loss갱신
        ...
        print(f"train loss: {train_loss:.5f}, "
              ...
              f"val acc: {val_acc:.1f} %, time: {time.time()-epoch_start:.0f} s") #한 epoch당 몇초 걸렸는지 확인 가능
        print("-"*20)
...
def loss_epoch(model, DL, criterion, optimizer = None): #train loss와 val loss포함
    N = len(DL.dataset) # the number of data
    rloss = 0; rcorrect = 0
    for x_batch, y_batch in tqdm(DL, leave=False): #tqdm(DL, position=10, leave=False): # position은 줄바꿈 개수, tqdm으로 dataloader감싸면 학습 varbose잘 보여줌
        ...
        # update
        if optimizer is not None: #val,test떄문 optimizer =None으로 놓고 학습업데이트 x 
